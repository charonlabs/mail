...
def cmd_webhook_get(args: Namespace) -> None:
    """
    Get a specific webhook by ID on the MAIL server.
    """
    ...
    # 2. Attempt to get the specific webhook by ID on the MAIL server
    response = httpx.get(
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"get webhook request to {MAIL_SERVER} failed with status code {response.status_code}"
        )

    response_json = response.json()
    try:
        response_obj = AdminWebhookGetResponse.model_validate(response_json)
...
def _print_json(response_obj: AdminWebhookGetResponse) -> None:
    print(response_obj.model_dump_json())


def _print_text(response_obj: AdminWebhookGetResponse) -> None:
