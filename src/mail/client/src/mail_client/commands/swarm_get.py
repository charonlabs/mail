...
def cmd_swarm_get(args: Namespace) -> None:
    """
    Get a specific swarm by name on the MAIL server.
    """
    ...
    # 2. Attempt to get the specific swarm by name on the MAIL server
    response = httpx.get(
        url=f"{MAIL_SERVER}/swarms/{args.swarm_name}",
        headers={
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"get swarm request to {MAIL_SERVER} failed with status code {response.status_code}"
        )

    response_json = response.json()
    try:
        response_obj = GetSwarmResponse.model_validate(response_json)
...
def _print_json(response_obj: GetSwarmResponse) -> None:
    print(response_obj.model_dump_json())


def _print_text(response_obj: GetSwarmResponse) -> None:
