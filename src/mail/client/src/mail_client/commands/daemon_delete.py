...
def cmd_daemon_delete(args: Namespace) -> None:
    """
    Delete a specific daemon by local address on the MAIL server.
    """
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"delete daemon request to {MAIL_SERVER} failed with status code {response.status_code}"
        )

    response_json = response.json()
    try:
        response_obj = AdminDaemonDeleteResponse.model_validate(response_json)
...
def _print_json(response_obj: AdminDaemonDeleteResponse) -> None:
    print(response_obj.model_dump_json())


def _print_text(response_obj: AdminDaemonDeleteResponse) -> None:
