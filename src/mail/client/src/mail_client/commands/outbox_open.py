...
def cmd_outbox_open(args: Namespace) -> None:
    """
    Open a specific message by ID in the MAIL user's outbox.
    """
    ...
    # 2. hit the server endpoint `GET /outbox/{message_id}`
    response = httpx.get(
        url=f"{MAIL_SERVER}/outbox/{args.message_id}",
        headers={
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"get outbox entry request to {MAIL_SERVER} failed with status code {response.status_code}"
        )
