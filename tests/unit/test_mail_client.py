...
async def test_mail_client_rest_endpoints() -> None:
	...
	async def handle_root(request: web.Request) -> web.Response:
		assert_auth(request)
		return web.json_response({"name": "mail", "status": "ok", "version": "1.0"})
	...
	async def handle_health(request: web.Request) -> web.Response:
		assert_auth(request)
		return web.json_response({"status": "healthy", "swarm_name": "demo", "timestamp": "now"})
	...
	async def handle_register_swarm(request: web.Request) -> web.Response:
		...
		return web.json_response({"status": "registered", "swarm_name": payload["name"]})
	...
	async def handle_interswarm_response(request: web.Request) -> web.Response:
		...
		return web.json_response({"status": "response_processed", "task_id": "task-001"})
	...
	async with run_app(app) as base_url:
		async with MAILClient(base_url, api_key="demo-token") as client:
			...
			await client.post_message("needs events", entrypoint="other", show_events=True)
			health = await client.get_health()
			swarms = await client.get_swarms()
			await client.register_swarm("test", "http://example.com", volatile=False, metadata={"label": "alpha"})
			...
			await client.send_interswarm_message("agent@remote", "hello", user_token="demo-user")
