...
async def main() -> None:
	...
	config = uvicorn.Config(server.app, host=base_host, port=base_port, log_level="error")
	...
	try:
		async with MAILClient(base_url, api_key="demo-token") as client:
			...
			message = await client.post_message("Ping from demo client", show_events=True)
